  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-20">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">Alerts & Incident Management</h1>
          <p className="text-gray-400 mt-1">Real-time disruption warnings and operational events</p>
        </div>
        ...
        <div className="flex gap-2">
          ...
          <button className="flex items-center gap-2 px-3 py-2 bg-[#0a0c14] hover:bg-white/5 text-gray-200 rounded-lg text-sm border border-white/10 transition-colors">
          ...
          <button
            ...
            className={\`px-4 py-2 rounded-lg text-sm font-medium uppercase tracking-wider whitespace-nowrap transition-colors \${
              activeTab === tab 
                ...
                : 'bg-[#0a0c14] text-gray-400 hover:bg-white/5 border border-white/5'
            }\`}
          >
            {tab}
          </button>
            ...
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
              <div className="flex-1">
                <div className="flex flex-wrap items-center gap-3 mb-2">
                  ...
                  <Badge variant="default" className="bg-[#0a0c14] border-white/10">{incident.type}</Badge>
                ...
                <div className="bg-[#05070a]/50 rounded-lg p-3 border border-white/5 flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-300">{incident.recommendedAction}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-row md:flex-col gap-2 shrink-0 w-full md:w-48">
                {incident.status === 'RESOLVED' ? (
                  ...
                  <>
                    ...
                    <button onClick={() => handleResolve(incident)} className="flex-1 md:flex-none px-4 py-2 bg-[#0a0c14] hover:bg-emerald-500/20 hover:text-emerald-400 border border-white/10 text-gray-200 text-sm font-medium rounded transition-colors text-center">
                      Mark Resolved
                    </button>
                  </>
                ) : (
                  <button onClick={() => handleAcknowledge(incident)} className="flex-1 md:flex-none px-4 py-2 bg-[#0a0c14] hover:bg-white/5 border border-white/10 text-gray-200 text-sm font-medium rounded transition-colors text-center">
                    Acknowledge
                  </button>
                )}
                
                <button onClick={() => navigate(\`/map?lat=\${incident.location[0]}&lng=\${incident.location[1]}&zoom=11\`)} className="flex-1 md:flex-none px-4 py-2 bg-cyan-500/10 hover:bg-cyan-500/20 border border-cyan-500/30 text-cyan-400 text-sm font-medium rounded transition-colors text-center">
                  View on Map
                </button>
              </div>
            </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => !submitting && setIsModalOpen(false)} title="Create Incident Alert">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Incident ID</label>
              <input type="text" value={formData.id} onChange={e => setFormData({...formData, id: e.target.value})} className={\`w-full bg-[#0a0c14] border \${errors.id ? 'border-red-500' : 'border-white/10'} rounded px-3 py-2 text-white\`} />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Type</label>
              <select value={formData.type} onChange={e => setFormData({...formData, type: e.target.value})} className="w-full bg-[#0a0c14] border border-white/10 rounded px-3 py-2 text-white">
          ...
          <div>
            ...
            <select value={formData.severity} onChange={e => setFormData({...formData, severity: e.target.value as any})} className="w-full bg-[#0a0c14] border border-white/10 rounded px-3 py-2 text-white">
          ...
          <div>
            ...
            <input type="text" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} className={\`w-full bg-[#0a0c14] border \${errors.title ? 'border-red-500' : 'border-white/10'} rounded px-3 py-2 text-white\`} />
          </div>

          <div>
            <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Location</label>
            <input type="text" value={formData.locationName} onChange={e => setFormData({...formData, locationName: e.target.value})} className={\`w-full bg-[#0a0c14] border \${errors.locationName ? 'border-red-500' : 'border-white/10'} rounded px-3 py-2 text-white\`} />
          </div>

          <div>
            <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Description / Impact</label>
            <textarea rows={3} value={formData.predictedImpact} onChange={e => setFormData({...formData, predictedImpact: e.target.value})} className={\`w-full bg-[#0a0c14] border \${errors.predictedImpact ? 'border-red-500' : 'border-white/10'} rounded px-3 py-2 text-white\`} />
          </div>
