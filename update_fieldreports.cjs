    ...
    <div className="max-w-[1920px] mx-auto space-y-8 pb-20">
      {settings.offlineMode && (
        <div className="bg-amber-500/20 border-l-4 border-amber-500 p-4 rounded-r-lg">
          <p className="text-amber-400 font-bold">Offline Mode Active — Changes will synchronize when connectivity is restored.</p>
        </div>
      )}
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-3 bg-gray-50 dark:bg-[#0a0c14] border border-gray-200 dark:border-white/5 rounded-lg px-4 py-2 shadow-inner">
            <div className="flex items-center gap-2 border-r border-gray-300 dark:border-white/10 pr-4">
              ...
              <span className={\`text-lg font-mono font-bold \${pendingCount > 0 ? 'text-amber-400' : 'text-emerald-400'}\`}>{pendingCount}</span>
            </div>
            <button 
              onClick={syncOfflineReports}
              disabled={pendingCount === 0}
              className={\`flex items-center gap-2 px-3 py-1.5 rounded text-xs font-bold uppercase tracking-widest transition-all \${pendingCount === 0 ? 'text-gray-600 cursor-not-allowed' : 'bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-400 shadow-[0_0_15px_rgba(8,145,178,0.3)]'}\`}
            >
          ...
          <Card key={report.id} className="group hover:border-cyan-500/30">
            <div className="flex justify-between items-start mb-4">
              <div className="flex items-center gap-3">
                ...
                <Badge variant="default" className="bg-gray-200 dark:bg-[#0a0c14] border-gray-300 dark:border-white/10">{report.id}</Badge>
              </div>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-gray-300 dark:border-white/5">
              <div className="text-xs text-gray-500 font-mono uppercase tracking-wider">
                Reporter: <span className="text-cyan-400">{report.officerName}</span>
              </div>
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => handleDelete(report.id)} className="px-3 py-1.5 text-xs font-bold text-red-400 hover:text-red-300 bg-red-500/10 hover:bg-red-500/20 rounded flex items-center gap-2 transition-colors">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input 
                ...
                className={\`w-full bg-[#0a0c14] border \${errors.id ? 'border-red-500' : 'border-white/10'} rounded px-3 py-2 text-white focus:outline-none focus:border-cyan-500\`}
              />
            ...
            <div>
              ...
              <input 
                ...
                className={\`w-full bg-[#0a0c14] border \${errors.officerName ? 'border-red-500' : 'border-white/10'} rounded px-3 py-2 text-white focus:outline-none focus:border-cyan-500\`}
              />
          ...
          <div>
            ...
            <input 
              ...
              className={\`w-full bg-[#0a0c14] border \${errors.locationName ? 'border-red-500' : 'border-white/10'} rounded px-3 py-2 text-white focus:outline-none focus:border-cyan-500\`}
            />
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select 
                ...
                className={\`w-full bg-[#0a0c14] border \${errors.incidentType ? 'border-red-500' : 'border-white/10'} rounded px-3 py-2 text-white focus:outline-none focus:border-cyan-500\`}
              >
            ...
            <div>
              ...
              <select 
                ...
                className={\`w-full bg-[#0a0c14] border \${errors.severity ? 'border-red-500' : 'border-white/10'} rounded px-3 py-2 text-white focus:outline-none focus:border-cyan-500\`}
              >
          ...
          <div>
            ...
            <textarea 
              ...
              className={\`w-full bg-[#0a0c14] border \${errors.description ? 'border-red-500' : 'border-white/10'} rounded px-3 py-2 text-white focus:outline-none focus:border-cyan-500\`}
            />
...
fs.writeFileSync('src/pages/FieldReports.tsx', code);
