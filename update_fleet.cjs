...
export default function FleetTracking() {
  ...
  const [formData, setFormData] = useState<Partial<Vehicle>>({
    id: '', type: 'Supply Truck', status: 'In Transit', currentRoute: { origin: '', destination: '', progress: 0 }, cargo: '', currentLocation: [0, 0]
  });
  ...
  const openAdd = () => {
    ...
    setFormData({
      ...
      type: 'Supply Truck',
      status: 'In Transit',
      currentRoute: { origin: '', destination: '', progress: 0 },
      cargo: '',
  ...
  const validate = () => {
    ...
    if (!formData.currentRoute?.origin) newErrors.origin = 'Origin is required.';
    if (!formData.currentRoute?.destination) newErrors.destination = 'Destination is required.';
  ...
  return (
    <div className="max-w-[1920px] mx-auto space-y-6 pb-20">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="border-l-4 border-l-cyan-500 dark:bg-[#0a0c14]/80 p-4" noPadding>
        ...
        <Card className="border-l-4 border-l-emerald-500 dark:bg-[#0a0c14]/80 p-4" noPadding>
        ...
        <Card className="border-l-4 border-l-amber-500 dark:bg-[#0a0c14]/80 p-4" noPadding>
        ...
        <Card className="border-l-4 border-l-red-500 dark:bg-[#0a0c14]/80 p-4" noPadding>
      ...
      <div className="bg-gray-50 dark:bg-[#0a0c14] border border-gray-300 dark:border-white/5 rounded-xl overflow-hidden shadow-2xl">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-gray-300 dark:border-white/5 bg-gray-200 dark:bg-white/5">
                ...
                <tr key={vehicle.id} className="hover:bg-gray-200 dark:hover:bg-white/5 transition-colors group">
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-lg bg-gray-300 dark:bg-white/5 border border-gray-400 dark:border-white/10 flex items-center justify-center shrink-0 group-hover:border-cyan-500/50 transition-colors">
                  ...
                  <td className="p-4">
                    <Badge variant={
                      ...
                      vehicle.status === 'Emergency' ? 'error' : 'default'
                    }>
                      {vehicle.status}
                    </Badge>
                  </td>
                  <td className="p-4">
                    <div className="flex items-center gap-3 w-48 lg:w-64">
                      ...
                      <div className="flex flex-col justify-between h-10 flex-1">
                        <div className="text-xs font-medium text-gray-900 dark:text-gray-300 truncate">{vehicle.currentRoute.origin}</div>
                        <div className="text-xs font-medium text-gray-900 dark:text-gray-300 truncate">{vehicle.currentRoute.destination}</div>
                      </div>
                    </div>
                  </td>
                  <td className="p-4">
                  ...
                  <td className="p-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => openEdit(vehicle)} className="p-2 text-gray-500 hover:text-cyan-400 hover:bg-cyan-500/10 rounded transition-colors" title="Edit">
                        <Edit className="w-4 h-4" />
                      </button>
                      <button onClick={() => handleDelete(vehicle.id)} className="p-2 text-gray-500 hover:text-red-400 hover:bg-red-500/10 rounded transition-colors" title="Delete">
      ...
      <Modal isOpen={isModalOpen} onClose={() => !submitting && setIsModalOpen(false)} title={editingVehicle ? 'Edit Vehicle' : 'Add Vehicle'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input 
                ...
                className={\`w-full bg-[#0a0c14] border \${errors.id ? 'border-red-500' : 'border-white/10'} rounded px-3 py-2 text-white focus:outline-none focus:border-cyan-500 disabled:opacity-50\`}
              />
            ...
            <div>
              ...
              <select 
                ...
                className="w-full bg-[#0a0c14] border border-white/10 rounded px-3 py-2 text-white focus:outline-none focus:border-cyan-500"
              >
                <option value="Supply Truck">Supply Truck</option>
                <option value="Medical Transport">Medical Transport</option>
                <option value="Recon Vehicle">Recon Vehicle</option>
          ...
          <div>
            <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Cargo Type *</label>
            <input 
              type="text" 
              placeholder="e.g. Medical Supplies, Rations"
              value={formData.cargo} 
              onChange={e => setFormData({...formData, cargo: e.target.value})}
              className={\`w-full bg-[#0a0c14] border \${errors.cargo ? 'border-red-500' : 'border-white/10'} rounded px-3 py-2 text-white focus:outline-none focus:border-cyan-500\`}
            />
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input 
                type="text" 
                value={formData.currentRoute?.origin || ''} 
                onChange={e => setFormData({...formData, currentRoute: {...formData.currentRoute!, origin: e.target.value}})}
                className={\`w-full bg-[#0a0c14] border \${errors.origin ? 'border-red-500' : 'border-white/10'} rounded px-3 py-2 text-white focus:outline-none focus:border-cyan-500\`}
              />
            ...
            <div>
              ...
              <input 
                type="text" 
                value={formData.currentRoute?.destination || ''} 
                onChange={e => setFormData({...formData, currentRoute: {...formData.currentRoute!, destination: e.target.value}})}
                className={\`w-full bg-[#0a0c14] border \${errors.destination ? 'border-red-500' : 'border-white/10'} rounded px-3 py-2 text-white focus:outline-none focus:border-cyan-500\`}
              />
          ...
          <div>
            ...
            <select 
              ...
              className={\`w-full bg-[#0a0c14] border \${errors.status ? 'border-red-500' : 'border-white/10'} rounded px-3 py-2 text-white focus:outline-none focus:border-cyan-500\`}
            >
...
fs.writeFileSync('src/pages/FleetTracking.tsx', code);
